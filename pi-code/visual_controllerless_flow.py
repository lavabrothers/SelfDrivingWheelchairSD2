"""
visual_controllerless_flow.py

This module implements the main asynchronous control flow for the self-driving
wheelchair, specifically designed for controllerless operation with integrated
real-time visual feedback. It combines various autonomous modes (cruise, follow,
mapping) with terminal command input and displays live video feeds or status screens
using OpenCV.
...
Key Features:
- Integrates terminal command input for mode selection and manual control.
...
- Supports environmental mapping with a 360-degree scan, which can be interrupted.
...
async def mapping_loop():
    ...
    This loop initiates an environmental mapping sequence. It sets a dedicated
    "Mapping in Progress" screen as the visual feedback. The mapping process
    ...
    while True:
        if current_state == ControlState.MAPPING:
            ...
            # This version does NOT shut down the vision module during mapping.
...
async def visual_flow_loop():
    ...
    It also handles the 'q' key press to initiate shutdown.
    """
    global current_visual_frame 
...
async def terminal_input_loop():
    ...
    and joystick-like inputs. It also handles graceful shutdown on EOF or KeyboardInterrupt.
    """
    ...
    while True:
        try:
            ...
            print("\nInput loop stopped. Shutting down...")
            if not loop.is_stopping():
...
async def main():
    """
    Main asynchronous entry point for the visual controllerless wheelchair control system.
    ...
    mapping, visual display, terminal input), and manages the overall state and
    cleanup process. It includes a safety timeout for manual mode.
    """
    ...
    try:
        ...
        # 3. Initialize vision module for person and obstacle detection.

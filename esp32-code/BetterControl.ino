...
const char* menuItems[] = {"Start Control", "Cruise Control", "Follow"};
                ...
                if (!joyMoved) {
                    ...
                    switch (selectedMenuItem) {
                        ...
                        case 1: // "Settings"
                            if (deviceConnected) {
                             // Send the BLE message
                                pTxCharacteristic->setValue("Cruise");
                                pTxCharacteristic->notify();
                                Serial.println("Sent BLE message: Cruise");

                                // Show confirmation
                                updateDisplay("Cruise Control", "Message Sent!", "Press Left to exit");
                        ...
                        case 2: // "Follow"
                            if (deviceConnected) {
                             // Send the BLE message
                                pTxCharacteristic->setValue("Follow");
                                pTxCharacteristic->notify();
                                Serial.println("Sent BLE message: Follow");

                                // Show confirmation
                                updateDisplay("Follow Message", "Message Sent!", "Press Left to exit");
                                currentMenuState = MENU_STATE_SUBPAGE;
                            } else {
                                // Show error if not connected
                               updateDisplay("Error", "BLE Not Connected", "Press Left to exit");
